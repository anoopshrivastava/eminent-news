...
exports.registerUser = catchAsyncError(async (req, res, next) => {
  ...
  if (!["user", "editor"].includes(role)) {
    return next(
      new Errorhandler("Role should be either 'user' or 'editor'", 402)
    );
  }
...
exports.loginUser = catchAsyncError(async (req, res, next) => {
  ...
  if (!email || !password) {
    // if entered empty email or password
...
exports.forgotPassword = catchAsyncError(async (req, res, next) => {
  ...
  try {
    ...
    // if some error occurs chang database reset token to undefine
...
exports.updateProfile = catchAsyncError(async (req, res, next) => {
  ...
  // If new avatar uploaded → replace old
  if (req.files && req.files.length > 0) {
    ...
    // Save new avatar
    updates.avatar = req.files[0].path;
  }

  // If avatar removal requested (explicit delete)
